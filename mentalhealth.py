...
class ChatInterface(Frame):
    ...
    # Options
        ...
        other.add_command(label="Physical Health Bot", command=self.botn)
       ...
       # color_theme.add_command(label='Mkbhd',command=self.MKBHD)
        ...
        help_option.add_command(label="About Ella", command=self.msg)
    ...
    def send_message_insert(self, message):
        ...
        pr1 = "You : " + user_input + "\n"
        if "physical health" in user_input.lower():
            self.text_box.configure(state=NORMAL)
            ...
            #t1.start()
            #time.sleep(1)
            ob="Opening Physical Health Bot..."
            pr="Ella : " + ob + "\n"
            self.text_box.configure(state=NORMAL)
            self.text_box.insert(END, pr)
            self.text_box.configure(state=DISABLED)
...
root.title("Mental Health Care Chatbot")
